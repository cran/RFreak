...
setClass("ltsEA", representation("FreakReturn",best="numeric",coefficients="numeric",crit="numeric"))
...
.checkData <- function(resp, preds,allowNegative=FALSE){ 
	...
	if (n2 < 1) stop("data mandatory")
   	if (sum(preds<0) != FALSE) stop("some negative data among the predictors")        
}
...
LTSevol <- function(y,x,h=NULL,adjust=FALSE,runs=1,generations=10000){
	...
	if (is.null(h)) h=floor((sum(dim(matrix))+1)/2.0)
	runsJava = as.integer(runs);
	generationJava = as.integer(generations);
	hJava=as.integer(h)
	adjustJava=as.logical(adjust)
	schedule<-.jcall("freak/rinterface/model/ScheduleConfigurator","Lfreak/core/control/Schedule;","getLTSSchedule",dataObject,hJava,adjustJava,runsJava,generationJava);
	...
	.jcall("freak/module/searchspace/PointSet","V","setPointsSetFromR",as.logical(TRUE));			
	return(new("ltsEA",summary=returnedFrame,best=best,coefficients=coefficients,crit=crit))	
}

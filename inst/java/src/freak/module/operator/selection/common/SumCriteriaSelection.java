 ...
 *
 */
public class SumCriteriaSelection extends Selection implements Configurable, CompatibleWithDifferentSearchSpaces {
	 ...
	 */
	public SumCriteriaSelection(OperatorGraph graph) {
		super(graph);
		// Add ports
		super.addInPort();
		super.addOutPort();
	...
	public void testSchedule(Schedule schedule) throws UnsupportedEnvironmentException {
		...
		if (!(graph.getSchedule().getFitnessFunction() instanceof MultiObjectiveFitnessFunction)) {
			throw new UnsupportedEnvironmentException("This operator works on abstract multi objective fitness functions only.");
		}
	...
	public IndividualList[] process(IndividualList[] original) {
		...
		for (int i = 0; i < origin.length; i++) {
			fValue[i] = ((MultiObjectiveFitnessFunction)fitness).evaluate(origin[i],original[0])[0];
			fValue[i] += ((MultiObjectiveFitnessFunction)fitness).evaluate(origin[i],original[0])[1];
	...
	public String getName() {
		return "Sum Criteria Selection";
	}
	
	public String getDescription() {
		return "Selects the best individuals according to the sum of the first two criteria of a multi value fitness function. " + "Individuals coming from this generation are preferred if their " + "fitness values equal.";
	}

 ...
 *
 */
public class ControlSelection extends Selection implements Configurable {
	 ...
	 */
	public ControlSelection(OperatorGraph graph) {
		super(graph);
		// Add ports
		super.addInPort();
		super.addOutPort();
	...
	public IndividualList[] process(IndividualList[] original) {
		...
		for (int i = 0; i < origin.length; i++) {
			...
			if ((double)bfg.evaluate0s() / (double)bfg.getNum0Rows() >= threshold / 100) {
	...
	public String getShortDescriptionForThreshold() {
		return "Min. for fulfilled 0-lines.";
	}
	
	public String getLongDescriptionForThreshold() {
		return "Minimum percentage of fulfilled 0-lines for selected individuals.";
	}
	...
	public String getDescription() {
		return "Selects all individuals whose DNFTree fulfilles more than threshold percent of the 0-lines.";
	}
}
